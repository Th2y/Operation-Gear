...
public class EnemyController : MonoBehaviour, IAgentObserver
{
    ...
    void Awake()
    {
        ...
        enemyTarget.player = player.transform;
        enemyTarget.Follow();
        ...
        if (!isDead)
        {
            if (isDashing)
            {
                ...
                else
                {
                    ...
                    if (distance <= 1)
                    {
                        float playerDistance = Vector2.Distance(agent.TargetPosition, enemyTarget.transform.position);
                        if(playerDistance <= Mathf.Epsilon)
                        {
                            Vector2 pushDirection = (agent.TargetPosition - this.transform.position).normalized;
